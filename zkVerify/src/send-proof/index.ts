...
const main = async (): Promise<void> => {
  ...
  if (!proofType) {
    throw new Error(
      "Proof type argument is required. Usage: npm run generate:single:proof <proof-type> <skipAttestation>"
    );
  }
  ...
  try {
    ...
    const { proof, publicSignals, vk } = await generateAndNativelyVerifyProof(
      proofType
    );
    console.log(`${proofType} Proof generated and natively verified.`);

    const proofParams = [{ Vk: vk }, proof, publicSignals];
    ...
    console.log(
      `Sent 1 proof, elapsed time: ${elapsedTime}s, result: ${result.result}, attestationId: ${result.attestationId}`
    );
  } catch (error) {
